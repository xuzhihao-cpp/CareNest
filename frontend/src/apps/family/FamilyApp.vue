...
type FamilyTab = 'profile' | 'binding' | 'address' | 'orders' | 'reports';
...
async function loadUser() {
  ...
  if (response.code === 0) user.value = response.data;
}
...
<template>
  <view class="user-app">
    <view class="user-header">
      <view><text class="kicker">FAMILY CARE</text><text class="title">{{ user?.displayName || '家属服务' }}</text></view>
      <button type="button" class="logout" @click="signOut">退出</button>
    </view>
