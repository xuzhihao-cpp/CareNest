...
type FamilyPrimaryView = 'home' | 'services' | 'reports' | 'account';
type FamilyTab = 'profile' | 'health' | 'feedback' | 'medical' | 'binding' | 'address' | 'orders' | 'reports' | 'service-feedback';
...
function selectPrimary(next: FamilyPrimaryView) {
  ...
  if (next === 'reports' && !['reports', 'service-feedback'].includes(activeTab.value)) activeTab.value = 'reports';
      ...
      <transition v-else name="view-shift" mode="out-in">
        <view :key="primaryView" class="primary-view">
          ...
          <template v-else-if="primaryView === 'reports'">
            <nav class="section-nav two-columns" aria-label="服务跟进功能">
