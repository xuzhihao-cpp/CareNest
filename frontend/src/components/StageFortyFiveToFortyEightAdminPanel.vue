...
async function decideComplaint(status: 'RESOLVED' | 'REJECTED') {
  ...
  if (response.code !== 0) {
    error.value = response.code === 409 ? '投诉状态已被其他人员更新，请刷新。' : '投诉处理未完成，请稍后重试。';
    return;
  }
...
async function decideAppeal(decision: 'APPROVED' | 'REJECTED') {
  ...
  if (response.code !== 0) {
    error.value = response.code === 409 ? '该申诉已由其他人员处理，请刷新。' : '申诉审核未完成，请稍后重试。';
    return;
  }
