...
<template>
  ...
  <view v-else-if="canViewOwnReports" class="stage-fifteen-panel glass-panel user-report-panel" aria-label="我的服务报告">
...
.report-picker-layout { display: grid; grid-template-columns: minmax(280px, 360px) minmax(0, 1fr); align-items: start; gap: 18px; min-width: 0; }.report-order-list { display: grid; align-content: start; grid-auto-rows: min-content; grid-template-columns: 1fr; gap: 10px; min-width: 0; max-height: calc(100vh - 250px); overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; }.report-list-heading { position: sticky; top: 0; z-index: 1; grid-column: 1; padding: 2px 0 10px; background: #fff; color: #254740; font-size: 16px; font-weight: 700; }.report-list-heading text:last-child { color: #72837f; font-size: 13px; font-weight: 400; }.report-order-card { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 10px; align-items: center; width: 100%; min-height: 90px; padding: 13px; border: 1px solid rgba(120, 144, 156, .16); border-radius: 8px; background: #fff; color: #40505d; text-align: left; }.report-order-card.active { border-color: rgba(11,143,157,.38); background: #f0faf7; }.report-order-card .flow-time { margin-top: 4px; }
...
.user-report-panel .report-picker-layout { grid-template-columns: 1fr; }
.user-report-panel .report-order-list { max-height: min(42vh, 420px); }
.user-report-panel .report-content { grid-template-columns: 1fr; }
@media (max-width: 900px) { .report-picker-layout { grid-template-columns: 1fr; }.report-order-list { max-height: min(48vh, 460px); }.report-preview, .report-content { grid-template-columns: 1fr; } }
</style>
