...
<template>
  <view class="dashboard-panel">
    ...
    <template v-else-if="active === 'basic' && basic">
      ...
      <section class="trend"><h3>订单趋势</h3><view v-for="point in basic.orderTrend" :key="point.date" class="trend-row"><text>{{ point.date }}</text><view><i :style="{ width: barWidth(point.value, basic.orderTrend) }" /></view><strong>{{ point.value }}</strong></view><view v-if="!basic.orderTrend.length" class="state">当前日期范围暂无订单趋势。</view></section>
    </template>
