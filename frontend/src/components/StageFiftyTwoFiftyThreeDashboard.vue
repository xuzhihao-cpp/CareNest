...
<template>
  <view class="dashboard-panel">
    ...
    <template v-else-if="active === 'basic' && basic">
      ...
      <section class="trend"><h3>订单趋势</h3><view v-if="basic.orderTrend.length" class="trend-summary"><view v-for="point in basic.orderTrend" :key="point.date" class="trend-summary-card"><strong>{{ point.value }} 单</strong><text>{{ shortDate(point.date) }}</text></view></view><view v-else class="state">当前日期范围暂无订单趋势。</view></section>
    </template>
