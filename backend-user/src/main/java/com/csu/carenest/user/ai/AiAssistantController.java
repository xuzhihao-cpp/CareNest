...
public class AiAssistantController {
    ...
    private final AiSpeechTranscriptionService transcriptionService;
    public AiAssistantController(AiAssistantService service, AiSpeechTranscriptionService transcriptionService){
    ...
    @PostMapping(value="/ai/speech/transcriptions", consumes=MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponse<AiAssistantDtos.SpeechTranscription> transcribe(
            @RequestHeader(value="Authorization",required=false) String a,
            @RequestParam(value="elderId",required=false) String elderId,
            @RequestPart("audio") MultipartFile audio) {
        return ApiResponse.success(transcriptionService.transcribe(a, elderId, audio));
    }
