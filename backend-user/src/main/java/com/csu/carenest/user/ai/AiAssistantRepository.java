...
public class AiAssistantRepository {
    ...
    public List<AiAssistantDtos.SessionSummary> sessionsForElder(String elderId,String ownerId,int limit,int offset) { return jdbc.query("SELECT s.session_id,s.elder_id,e.elder_name,s.session_title,s.session_status,s.safety_level,(SELECT m.content_summary FROM ai_assistant_message m WHERE m.session_id=s.session_id ORDER BY m.created_at DESC,m.message_id DESC LIMIT 1),s.created_at,s.updated_at FROM ai_assistant_session s JOIN elder_profile e ON e.elder_id=s.elder_id WHERE s.elder_id=? AND s.user_id=? ORDER BY s.updated_at DESC LIMIT ? OFFSET ?",(r,n)->new AiAssistantDtos.SessionSummary(r.getString(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7),r.getString(8),r.getString(9)),elderId,ownerId,limit,offset); }
    public long sessionCountForElder(String elderId,String ownerId) { return jdbc.queryForObject("SELECT COUNT(*) FROM ai_assistant_session WHERE elder_id=? AND user_id=?",Long.class,elderId,ownerId); }
    public List<AiAssistantDtos.SessionSummary> sessionsForFamily(String familyId,String elderId,int limit,int offset) { return jdbc.query("SELECT s.session_id,s.elder_id,e.elder_name,s.session_title,s.session_status,s.safety_level,(SELECT m.content_summary FROM ai_assistant_message m WHERE m.session_id=s.session_id ORDER BY m.created_at DESC,m.message_id DESC LIMIT 1),s.created_at,s.updated_at FROM ai_assistant_session s JOIN elder_profile e ON e.elder_id=s.elder_id JOIN elder_family_binding b ON b.elder_id=s.elder_id AND b.family_id=? AND b.binding_status='ACTIVE' WHERE (? IS NULL OR s.elder_id=?) ORDER BY s.updated_at DESC LIMIT ? OFFSET ?",(r,n)->new AiAssistantDtos.SessionSummary(r.getString(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7),r.getString(8),r.getString(9)),familyId,elderId,elderId,limit,offset); }
