...
public class AiAssistantService {
    ...
    public AiAssistantDtos.PageResult<AiAssistantDtos.SessionSummary> listSessions(String token,String elderId,int page,int size){AuthService.CurrentUser u=auth.requireCurrentUser(token);if(page<1||size<1||size>50)throw new ApiException(422,"invalid page");if(u.roles().contains(RoleCode.ELDER)){AiAssistantRepository.Elder elder=repo.elderByUser(u.userId()).orElseThrow(()->new ApiException(404,"elder not found"));return new AiAssistantDtos.PageResult<>(repo.sessionsForElder(elder.id(),u.userId(),size,(page-1)*size),repo.sessionCountForElder(elder.id(),u.userId()),page,size);}if(u.roles().contains(RoleCode.FAMILY))return new AiAssistantDtos.PageResult<>(repo.sessionsForFamily(u.userId(),elderId,size,(page-1)*size),repo.sessionCountForFamily(u.userId(),elderId),page,size);throw new ApiException(403,"role not allowed");}
