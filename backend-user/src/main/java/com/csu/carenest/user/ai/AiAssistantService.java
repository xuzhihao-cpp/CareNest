...
public class AiAssistantService {
    private final AuthService auth; private final AiAssistantRepository repo; private final AiProvider provider; private final FamilyAssistanceIntentDetector assistanceIntent;
    public AiAssistantService(AuthService auth,AiAssistantRepository repo,AiProvider provider,FamilyAssistanceIntentDetector assistanceIntent){this.auth=auth;this.repo=repo;this.provider=provider;this.assistanceIntent=assistanceIntent;}
    ...
    @Transactional public AiAssistantDtos.MessageResult message(String token,String sessionId,AiAssistantDtos.MessageRequest req){
        ...
        boolean familyAssistanceRequested = u.roles().contains(RoleCode.ELDER) && assistanceIntent.detects(req.content());
        return new AiAssistantDtos.MessageResult(sessionId,userMsg,aiMsg,result.answer(),result.safetyLevel(),!"NORMAL".equals(result.safetyLevel()),assistance,cs,familyAssistanceRequested);
    }
