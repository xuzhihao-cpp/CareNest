...
| 泳道 | 主要职责 | 关键目录 |
| --- | --- | --- |
| 数据库 / 数据规范 | 数据字典、状态字典、表结构、初始化数据、ER 图 | `db/`, `docs/data-dictionary.md` |
...
- 建立 `docs/data-dictionary.md` 作为唯一人工维护的数据字典源。
- 冻结字段命名、数据库列命名和核心枚举值。
...
| 目录 | 说明 |
| --- | --- |
| `backend-user/` | 用户侧后端工程目录，阶段 1-2 只保留目录说明。 |
| `backend-care-admin/` | 护理端与管理端后端工程目录，阶段 1-2 只保留目录说明。 |
| `frontend/` | uni-app 前端工程目录，阶段 1-2 只保留目录说明。 |
| `db/` | 数据库脚本、ER 图、初始化数据目录，阶段 3 开始写 SQL。 |
...
- 数据库字段使用 `snake_case`。
- 枚举值使用大写英文。
- 任何被两个以上模块使用的字段，必须先进入 `docs/data-dictionary.md`。
