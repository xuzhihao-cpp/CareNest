...
| phase-16 | ReportAckRequest | ackResult | ack_result | 确认结果 | string | yes | ackResult | 长辈或家属确认报告结果 |
...
| order | NursingOrder | addressId | address_id | 服务地址 ID | string | yes |  | 阶段 10 下单使用 |
| order | NursingOrder | scheduledStartAt | scheduled_start_at | 预约开始时间 | datetime | yes |  | 阶段 10 下单使用 |
| order | NursingOrder | scheduledEndAt | scheduled_end_at | 预约结束时间 | datetime | no |  | 由服务时长推导或前端选择 |
| order | NursingOrder | servicePriceCent | service_price_cent | 服务价格分 | integer | yes |  | 下单时快照价格 |
| order | NursingOrder | contactName | contact_name | 联系人姓名 | string | yes |  | 上门联系信息 |
| order | NursingOrder | contactPhone | contact_phone | 联系人电话 | string | yes |  | 上门联系信息 |
| order | NursingOrder | remark | remark | 备注 | string | no |  | 下单备注 |
| order | NursingOrder | createdBy | created_by | 创建人用户 ID | string | no |  | 下单操作人 |
| order | OrderStatusLog | statusLogId | status_log_id | 订单状态日志 ID | string | yes |  | 状态日志主键 |
| order | OrderStatusLog | fromStatus | from_status | 变更前状态 | string | no | orderStatus | 初始创建可为空 |
