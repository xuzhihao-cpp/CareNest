...
RUN --mount=type=cache,id=carenest-maven,target=/root/.m2 \
    mvn -B -ntp -pl backend-user -am dependency:go-offline

COPY backend-user/src backend-user/src
RUN --mount=type=cache,id=carenest-maven,target=/root/.m2 \
    mvn -B -ntp -pl backend-user -am -DskipTests package
...
WORKDIR /app
COPY --from=build --chown=carenest:carenest /workspace/backend-user/target/backend-user-*.jar /app/app.jar
USER carenest
EXPOSE 8081
ENV JAVA_TOOL_OPTIONS="-XX:MaxRAMPercentage=75.0 -Dfile.encoding=UTF-8 -Duser.timezone=Asia/Shanghai"
HEALTHCHECK --interval=10s --timeout=5s --start-period=30s --retries=12 \
  CMD curl -fsS http://127.0.0.1:8081/api/v1/health | grep -q '"dbConnected":true' || exit 1
